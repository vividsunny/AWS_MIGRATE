	...
	*/
	...
	if ( ! $error ) {
		$file_url = $upload['file'];
		 ...
		 // $file_url = 'c://xampp/htdocs/woo-demo/wp-content/uploads/2020/06/Book1-1.csv'; 
		 // $file_url = 'c://xampp/htdocs/woo-demo/wp-content/uploads/2020/06/price-guide.csv'; 
		...
		<script type="text/javascript">
			jQuery(document).ready(function(){
				...
				if( url != ''){
				    script_wp_video_games( start_pos, url );
				}
...
<script type="text/javascript">
	function script_wp_video_games(pos,fileurl=''){
		jQuery.ajax({
			url: '<?php echo admin_url( 'admin-ajax.php' ); ?>',
			type: 'POST',
			dataType: 'json',
			data: {
				action : 'video_games_import_script',
				startpos: pos,
				file_url : fileurl,
			},
			success: function( response ) {
				...
				if ( response.success ) {
					...
					if(newpos == 'done'){
						...
						jQuery('.import_response').prepend(message +'</br>' );
						script_wp_video_games(newpos,fileurl);
					}
				}else{
