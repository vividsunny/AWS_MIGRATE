...
if( !class_exists( 'va_aws' ) ){
	...
	class va_aws 
	{
	     ...
	     */
	    private $aws;
	    private $buyseason_dir;
	    public function __construct($access_key,$secret_key)
	    {
			require POPUP_PLUGIN_DIR.'include/aws/vendor/autoload.php';
	    ...
	    public function aws_file_upload($Fieldname,$bucketname){
	    	...
	    	$this->buyseason_dir = $upload_dir['basedir'].'/buyseason-img/';
	    	...
	    	if(isset( $_FILES[$Fieldname] ) && $_FILES[$Fieldname] != ''){
	    		...
	    		$file_name     = wp_basename( $temp_file );
	     		...
	     		$upload_path = $this->buyseason_dir.$file_name.$ext;
	    ...
	    public function aws_resize_image($file_name,$ext,$s3path,$orignal_file_path){
	    	...
	    	foreach( $dimentions as  $sizename => $dimention){
				...
				$dest_path = $this->buyseason_dir.$resized_file;
	    ...
	    public function aws_file_upload_url($url,$bucket_name){
		    ...
		    $unique_file_name = wp_unique_filename( $this->buyseason_dir, $image_name ); // Generate unique name
		    $filename         = basename( $unique_file_name ); // Create image file name

		    // Check folder permission and define file location
		    if( wp_mkdir_p( $this->buyseason_dir ) ) {
		        $file = $this->buyseason_dir.$filename;
		    } else {
		        $file = $this->buyseason_dir.$filename;
		    }
