...
class WC_Authorize_Net_CIM_Shipping_Address {
	...
	public function matches_address( $address ) {
		...
		if ( ! empty( $address['country'] ) ) {
			$address['country'] = 3 === strlen( $address['country'] ) ? Framework\SV_WC_Helper::convert_country_code( $address['country'] ) : $address['country'];
		} else {
			$address['country'] = '';
		}
	...
	protected function get_shipping_from_object( $object ) {
		...
		if ( $object instanceof WC_Order ) {
			...
			$shipping = array(
				'first_name' => Framework\SV_WC_Order_Compatibility::get_prop( $object, 'shipping_first_name' ),
				'last_name'  => Framework\SV_WC_Order_Compatibility::get_prop( $object, 'shipping_last_name' ),
				'company'    => Framework\SV_WC_Order_Compatibility::get_prop( $object, 'shipping_company' ),
				'address'    => trim( Framework\SV_WC_Order_Compatibility::get_prop( $object, 'shipping_address_1' ) . ' ' . Framework\SV_WC_Order_Compatibility::get_prop( $object, 'shipping_address_2' ) ),
				'city'       => Framework\SV_WC_Order_Compatibility::get_prop( $object, 'shipping_city' ),
				'state'      => Framework\SV_WC_Order_Compatibility::get_prop( $object, 'shipping_state' ),
				'postcode'   => Framework\SV_WC_Order_Compatibility::get_prop( $object, 'shipping_postcode' ),
				'country'    => Framework\SV_WC_Order_Compatibility::get_prop( $object, 'shipping_country' ),
			);
