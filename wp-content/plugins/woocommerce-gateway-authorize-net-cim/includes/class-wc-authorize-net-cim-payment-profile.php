...
class WC_Authorize_Net_CIM_Payment_Profile extends Framework\SV_WC_Payment_Gateway_Payment_Token {
	...
	protected function get_billing_from_order( WC_Order $order ) {
		...
		return array(
			...
			'address'    => trim( Framework\SV_WC_Order_Compatibility::get_prop( $order, 'billing_address_1' ) . ' ' . Framework\SV_WC_Order_Compatibility::get_prop( $order, 'billing_address_2' ) ),
	...
	protected function calculate_payment_hash( $data ) {
		...
		if ( empty( $data['billing']['postcode'] ) ) {
			...
			$billing_postcode = ( isset( $data['order'] ) && $data['order'] instanceof WC_Order ) ? Framework\SV_WC_Order_Compatibility::get_prop( $data['order'], 'billing_postcode' ) : get_user_meta( get_current_user_id(), 'billing_postcode', true );
