...
/**
 * The Authorize.Net CIM customer webhook handler.
 *
 * @since 2.8.0
 */
abstract class WC_Authorize_Net_CIM_Customer_Webhook extends WC_Authorize_Net_CIM_Webhook {
	...
	protected function mark_invalid_subscriptions( WC_Gateway_Authorize_Net_CIM $gateway, $customer_id, $token = '' ) {
		...
		// TODO: the 2.0+ check can be removed once CIM is upgraded to framework 5.0 {CW 2017-12-07}
		if ( ! $this->get_plugin()->is_subscriptions_active() || ! Framework\SV_WC_Plugin_Compatibility::is_wc_subscriptions_version_gte_2_0() ) {
			return;
		}
