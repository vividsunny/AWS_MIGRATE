...
class Payment_Processor extends Transaction_Processor {
    ...
    public function process( $params ) {
        ...
        $request = [
            ...
            'metadata'            => [ 'Order Number' => $params['order']->get_order_number() ],
        ];
        ...
        if ( get_option( 'woo_mp_stripe_include_name_and_email', 'yes' ) === 'yes' ) {
            $name  = $params['order']->get_billing_first_name() . ' ' . $params['order']->get_billing_last_name();
            $email = $params['order']->get_billing_email();

            $request['metadata']['Customer Name']  = $name;
            $request['metadata']['Customer Email'] = $email;
        }
