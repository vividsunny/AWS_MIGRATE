...
class WOO_MSTORE_admin_product {
	...
	public function init() {
		...
		add_action( 'delete_pa_color', array( $this, 'update_terms_mapping_on_term_delete' ) );
	...
	public function process_product_delete( $post_id ) {
		...
		$master_parent_product = 'variation' == $master_product->get_type() ? wc_get_product( $master_product->get_parent_id() ) : null;
...
//		$meta_keys[] = '_wpml_media_duplicate';
	...
	private function get_slave_product_mapped_image_id( $data, $master_product_image_id ) {
		...
		foreach ( (array) $master_product_image_id as $_master_product_image_id ) {
			...
			$wpdb->update(
				$wpdb->posts,
				array( 'post_content' => $post_content, 'post_excerpt' => $post_excerpt ),
	...
	private function synchronize_category_changes( $data ) {
		global $WOO_MSTORE;
	...
	public function update_terms_mapping_on_term_delete( $term_id ) {
		// get mapped terms
		$terms_mapping = get_option( 'terms_mapping', array() );
	...
	private function republish_slave_product_reviews( $data ) {
		global $wpdb;
	...
	// republish reviews
	private function republish_reviews( $master_comments, $master_blog_id ) {
		global $wpdb;
