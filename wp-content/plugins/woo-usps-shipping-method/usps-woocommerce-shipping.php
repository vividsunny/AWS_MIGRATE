<?php
/*
	Plugin Name: Elex WooCommerce USPS (BASIC) Shipping
...
function wf_usps_basic_pre_activation_check(){
	if ( is_plugin_active('usps-woocommerce-shipping/usps-woocommerce-shipping.php') ){
		...
		wp_die(__("Is everything fine? You already have the Premium version installed in your website. For any issues, kindly raise a ticket via <a target='_blank' href='https://elextensions.com/support/'>Elex Support</a>", "wf-usps-woocommerce-shipping"), "", array('back_link' => 1 ));
	}
}
	...
	if(! class_exists('USPS_WooCommerce_Shipping') ){ 
		class USPS_WooCommerce_Shipping {
			...
			public function scripts() {
				...
				wp_enqueue_style('wf_common_bootstrap',		plugins_url( '/resources/bootstrap.css',	__FILE__ ));
			}
		}
