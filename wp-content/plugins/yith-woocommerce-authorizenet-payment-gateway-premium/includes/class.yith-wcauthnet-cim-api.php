...
if ( ! class_exists( 'YITH_WCAUTHNET_CIM_API' ) ) {
	...
	class YITH_WCAUTHNET_CIM_API {
		...
		/**
		 * Checks whether a transaction is settled
		 *
		 * @param string $trans_id Transaction id.
		 * @return bool Whether transaction is settled or not.
		 */
		public function is_transaction_settled( $trans_id ) {
			try {
				$response = $this->retrieve_transaction( $trans_id );

				if ( ! empty( $response ) && isset( $response->transaction ) && isset( $response->transaction->transactionStatus ) ) {
					return 'settledSuccessfully' === (string) $response->transaction->transactionStatus;
				}
		...
		/**
		 * Retrieve object for a specific transaction
		 *
		 * @param string $trans_id Transaction id.
		 * @return \SimpleXMLElement|bool Parsed xml, or false on failure
		 * @throws Exception When error occurs in XML parsing.
		 */
		public function retrieve_transaction( $trans_id ) {
			$response = $this->do_request( $this->get_retrieve_transaction_xml( $trans_id ) );
		...
		public function get_create_refund_transaction_xml( $order, $amount, $payment_details ) {
			 ...
			 * @since 1.1.14
			 */
			$is_void = $order_total === $amount && ! $this->is_transaction_settled( $trans_id );
		...
		/**
		 * Returns xml string to retrieve a specific transaction on Authorize.net
		 *
		 * @param string $trans_id Transaction ID.
		 *
		 * @return string XML request
		 * @since 1.0.0
		 */
		public function get_retrieve_transaction_xml( $trans_id ) {
			// starts xml document.
			$this->xml_writer->openMemory();
			$this->xml_writer->startDocument( '1.0', 'UTF-8' );

			// <getTransactionDetailsRequest xmlns="AnetApi/xml/v1/schema/AnetApiSchema.xsd">
			$this->xml_writer->startElementNs( null, 'getTransactionDetailsRequest', 'AnetApi/xml/v1/schema/AnetApiSchema.xsd' );

			$this->add_auth_xml();

			$this->xml_writer->writeElement( 'transId', $trans_id );
