<?php
/**
 * This template displays Sent gift card table
 * 
 * This template can be overridden by copying it to yourtheme/wallet/premium/dashboard/gift-card-listing-sent.php
 * 
...
do_action ( 'hrw_before_sent_gift_card_listing' ) ;
...
if ( hrw_check_is_array ( $table_datas ) ) {
    ...
    $columns = array(
        ...
        esc_html__( 'Receiver Name' , HRW_LOCALE ) ,
        esc_html__( 'Gift Card' , HRW_LOCALE ) ,
        esc_html__( 'Amount' , HRW_LOCALE ) ,
        esc_html__( 'Reason' , HRW_LOCALE ) ,
        esc_html__( 'Purchased Date' , HRW_LOCALE ) ,
        esc_html__( 'Status' , HRW_LOCALE ) ,
        esc_html__( 'Redeemed Date' , HRW_LOCALE ) ,
        esc_html__( 'Expiry Date' , HRW_LOCALE ) ,
            ) ;
    ?>  
    <div class = "hrw_gift_sent_listing">
        <h2><?php echo esc_html__ ( 'Sent Gift Card(s)' , HRW_LOCALE ) ; ?></h2>
        <table class="hrw_gift_sent_listing_table hrw_frontend_table">
            <thead>
            ...
            <tbody>
                ...
                foreach ( $table_datas as $gift_id ) {
                    ...
                    ?>   
                    <tr>
                        <td><?php echo get_option( 'hrw_advanced_sno_type' , '1' ) == '1' ? absint( $serial_number ) : '#' . absint( $gift_id ) ; ?></td>
                        <td><?php echo esc_html( $gift_obj->get_receiver_display() ) ; ?></td> 
...
do_action ( 'hrw_after_sent_gift_card_listing' ) ;
