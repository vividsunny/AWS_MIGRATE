...
if (! function_exists('sc_shop_slider')) {
	function sc_shop_slider($attr, $content = null)
	{
		...
		##########New Code#############	
		} elseif ($show == 'pre-orderable' || $show == 'pre-orderable-grid') {
            
			// pre-orderable --------------------------------
			$args['post__in'] =  array_merge(array( 0 ), wc_get_product_ids_on_preorder());
		} elseif ($show == 'new-releases'  || $show == 'new-releases-grid') {
            
			// new-releases --------------------------------
			$args['post__in'] =  array_merge(array( 0 ), wc_get_product_ids_new_releases());			
			
			
		} elseif ($show == 'best-selling' || $show == 'best-selling-grid') {
        ##########End New Code#############	
			// best-selling --------------------------
		...
		$output = '<div class="shop_slider" data-count="'. esc_attr($query_shop->post_count) .'">';
			...
			$output .= '<ul class="shop_slider_ul">';
				...
				##########New Code#############
                    ...
                    if ( $show == 'pre-orderable' || $show == 'best-selling' || $show == 'new-releases') {
                                        
					$diamond = get_post_meta($product->get_id() ,"diamond_number", true);
                    $thumbnail_d = "https://s3.us-east-2.amazonaws.com/darksidecomics/zip-image/$diamond.jpg";
								...
								$output .= '<div class="image_frame scale-with-grid product-loop-thumb">';
									...
									$output .= '<div class="image_wrapper">';
										...
										$output .= '<a href="'. esc_url(get_the_permalink()) .'">';
											$output .= '<div class="mask"></div>';
											$output .= '<img src="'. esc_url($thumbnail_d) .'">';
										$output .= '</a>';
                    ...
                    }elseif ( $show == 'pre-orderable-grid' || $show == 'best-selling-grid' || $show == 'new-releases-grid') {
                    $diamond = get_post_meta($product->get_id() ,"diamond_number", true);
                    $thumbnail_d = "https://s3.us-east-2.amazonaws.com/darksidecomics/zip-image/$diamond.jpg";
